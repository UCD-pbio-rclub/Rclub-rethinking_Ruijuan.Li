...
distribution of samples, averaging over the posterior uncertaninty in p (? what does this mean). 
...
table(dummy_w_eight_fifteen)/1e5
```

3M4. Using the posterior distribution constructed from the new (8/15) data, now calcualte the 

probability of observing 6 water in 9 tosses. (don't understand the question, use this posterior 

distribution as what? draw samples from this posterior distibution? view this distribution as a 

model? )

```{r}
dummy_w_six_nine_new <- rbinom(1e5, size = 9, prob = samples)
table(dummy_w_six_nine_new)/1e5
# check the new 8/15 distribution
simplehist(dummy_w_eight_fifteen)
# define a grid 
p_grid <- seq(0, 1, length.out = 1e5)
# use new 8/15 as the posterior
posterior = dummy_w_eight_fifteen
posterior = dummy_w_eight_fifteen/sum(dummy_w_eight_fifteen)
# draw samples from posterior
set.seed(100)
samples <- sample(p_grid, prob = posterior, size = 1e4, replace = TRUE)
# sampling to simulate prediction
dummy_w_six_nine <- rbinom(1e5, size = 9, prob = samples)
table(dummy_w_six_nine)/1e5
```
...
helps, compare inferences (using both priors) to the true value p=0.7 (I feel there is very little 

difference made with different prior...)
...
dummy_w_eight_fifteen_new <- rbinom(1e5, size = 15, prob = samples)
table(dummy_w_eight_fifteen_new)/1e5

# 6/9, with this original 8/15 posterior
dummy_w_six_nine_new <- rbinom(1e5, size = 9, prob = samples)
table(dummy_w_six_nine_new)/1e5

# with 8/15 posterior from simulated data
p_grid <- seq(0, 1, length.out = 1e5)
posterior <- dummy_w_eight_fifteen_new
posterior <- posterior/sum(posterior)
set.seed(100)
samples <- sample(p_grid, prob = posterior, size = 1e5, replace = TRUE)
dummy_w_six_nine_new_new <- rbinom(1e5, size = 9, prob = samples)
table(dummy_w_six_nine_new_new)/1e5
```
...
p_grid <- seq(0, 1, length.out = 1000)
prior <- rep(1, 1000)
likelihood <- dbinom(1, size = 1, prob = p_grid) # also binomial distribution??? I don't think 
# so!!! So this might be wrong!!!  
posterior <- likelihood * prior
posterior <- posterior/sum(posterior)
```
```{r,echo=FALSE}
